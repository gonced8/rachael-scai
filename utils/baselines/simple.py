...
def retrieve(turn, rewritten_question, searcher, num_passages):
    ...
    return [
        {"id": hit.docid, "score": hit.score, "text": json.loads(hit.raw)["contents"]}
        for hit in hits
    ]
...
def answer(turn, rewritten_question, retrieved_passages):
    ...
    for passage in retrieved_passages:
        ...
        for passage_sentence in passage_blob.sentences:
            if passage_sentence.ends_with(
                "."
            ):  # simple sentence check: should end in a "."
                passage_sentence_phrases = passage_sentence.noun_phrases.stem()
                score = len(
                    [
                        phrase
                        for phrase in passage_sentence_phrases
...
def run_for_turn(turn, conversation, searcher, num_passages):
    ...
    result = {
        ...
        "Model_passages": {
            passage["id"]: passage["score"] for passage in retrieved_passages
        },
