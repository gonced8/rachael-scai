    ...
    for opt, arg in opts:
        if opt == "--input-dataset":
            ...
            if arg.endswith(".json"):
                options["ground-truth"] = load_ground_truth(arg)
            else:
                options["ground-truth"] = load_ground_truth(
                    arg + "/" + input_dataset_file_name
                )
    ...
    for turn in tqdm(ground_truth):
        if eval_turn_one_rewrites or turn["Turn_no"] > 1:
            if eval_missing_truth or turn["Truth_rewrite"] != "":
                ...
                result[turn_id] = {"ROUGE1-R": score["rouge1"].mid.recall}
                rewrites = rewrites + 1
...
def get_retrieval_ground_truth(ground_truth, eval_missing_truth):
    ...
    for turn in ground_truth:
        ...
        if "Truth_passages" in turn and len(turn["Truth_passages"]) > 0:
            retrieval_ground_truth[turn_id] = {
                passage: 1 for passage in turn["Truth_passages"]
            }
...
def evaluate_retrieval(ground_truth, run, eval_missing_truth):
    ...
    if retrieval_run_for_type:  # at least one turn for this type => evaluate
        ...
        for turn_id, score in metric.evaluate(retrieval_run_for_type).items():
            result[turn_id] = {"MRR": score["recip_rank"]}

        # average_mrr = sum(mrrs) / len(mrrs)
        # result["MRR"] = average_mrr
        print(
            "    used retrieved passages for %d questions" % len(retrieval_run_for_type)
        )
    else:
    ...
    for turn in tqdm(ground_truth, desc="  "):
        ...
        if eval_missing_truth or turn["Truth_answer"] != "":
            ...
            score2 = metric2.compute(
                predictions=[answering_run[turn_id]], references=[turn["Truth_answer"]]
            )
            ...
            result[turn_id] = {
                ...
                "F1": score["f1"] / 100,
                "ROUGEL-F1": score2["rougeL"].mid.fmeasure,
            }
    if answers > 0:
        # score = metric.compute()
...
def evaluate(
    ground_truth,
    run,
    eval_missing_truth=False,
    eval_turn_one_rewrites=False,
    rewriting=True,
    retrieval=True,
    answering=True,
):
    ...
    if rewriting:
        merge(
            results,
            evaluate_rewriting(
                ground_truth, run, eval_missing_truth, eval_turn_one_rewrites
            ),
        )
    if retrieval:
        merge(results, evaluate_retrieval(ground_truth, run, eval_missing_truth))
    if answering:
        merge(results, evaluate_answering(ground_truth, run, eval_missing_truth))
    return results
...
def main(options):
    ...
    results = [{"turnid": turnid, **metrics} for turnid, metrics in results.items()]
    print(results)
    ...
    pd.DataFrame(results).to_csv(
        options["output-file-name"], sep="\t", float_format="%.6f", index=False
    )
